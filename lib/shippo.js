...
export async function createShippoLabel(addressFrom, addressTo, declaredValue = 0) {
  ...
  let shipment = await buildShipment(true)
  // Some carriers don't support signatureConfirmation — retry without it
  if (!shipment.rates?.length && sig) {
    shipment = await buildShipment(false)
  }
