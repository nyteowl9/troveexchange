...
import { AUTH_CENTER_ADDRESS } from './shippo'
...
export async function emailStaffSelfShipNoScan({ order, staffEmail }) {
  if (!staffEmail) return
  const shippedAt = new Date(order.shipped_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  const releaseDate = new Date(order.auto_release_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  const tracking = order.tracking_a
  const carrier  = order.self_ship_carrier
  return resend.emails.send({
    from: FROM, to: staffEmail,
    subject: `[Alert] Self-ship order — no carrier scan after 3 days — #${order.id.slice(0,8).toUpperCase()}`,
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:24px;font-weight:400;color:#F0EDE6;">No Carrier Scan — <em style="color:#E8A838;">Day 3 Alert</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 16px;">
        A self-ship order has had no carrier scan 3 days after the seller marked it shipped. This may indicate a fake tracking number or unshipped package.
      </p>
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;margin:0 0 20px;" cellpadding="8">
        ...
        <tr><td style="color:#6C6A66;font-size:13px;">Carrier</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${carrier || 'Not specified'}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Tracking</td><td style="color:#F0EDE6;font-size:13px;text-align:right;font-family:monospace;">${tracking || 'None provided'}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Shipped (seller claim)</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${shippedAt}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Auto-release</td><td style="color:#C84B3C;font-size:13px;text-align:right;font-weight:600;">${releaseDate}</td></tr>
      </table>
      <div style="background:rgba(232,168,56,0.08);border:1.5px solid rgba(232,168,56,0.3);border-radius:10px;padding:16px 20px;margin:0 0 24px;">
        <p style="color:#E8A838;font-size:13px;font-weight:600;margin:0 0 4px;">Recommended actions</p>
        <p style="color:#B8B4AC;font-size:13px;margin:0;line-height:1.6;">Look up the tracking number with the carrier directly. If it has never been scanned, consider contacting the seller and/or pausing the auto-release in the admin panel.</p>
      </div>
      ${btn('View Order in Admin', `${BASE_URL}/admin`)}
    `)
  })
}
  ...
  return resend.emails.send({
    ...
    html: layout(`
      ...
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;margin-bottom:8px;" cellpadding="8">
        ...
        <tr><td style="color:#6C6A66;font-size:13px;">Auth tier</td><td style="text-align:right;">${tag(
          order.auth_tier === 'physical' ? 'Physical Auth · $25'
            : order.auth_tier === 'remote' ? 'Remote Auth · $10'
            : 'No Auth · Free'
        )}</td></tr>
      </table>
  ...
  return resend.emails.send({
    ...
    html: layout(`
      ...
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;" cellpadding="8">
        <tr><td style="color:#6C6A66;font-size:13px;">Tracking</td><td style="color:#F0EDE6;font-size:13px;text-align:right;font-family:monospace;">${order.tracking_a || 'No tracking provided'}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Carrier</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${order.self_ship_carrier || 'Auto-selected (lowest rate)'}</td></tr>
      </table>
...
export async function emailSellerSaleShipNow({ to, order, sellerPayout }) {
  const payout = parseFloat(sellerPayout ?? order.seller_payout ?? 0)
  return resend.emails.send({
    from: FROM, to,
    ...
    html: layout(`
      ...
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;margin-bottom:8px;" cellpadding="8">
        ...
        <tr><td style="color:#6C6A66;font-size:13px;">Your payout</td><td style="color:#4CAF7C;font-size:14px;font-weight:700;text-align:right;">$${payout.toFixed(2)} USDC</td></tr>
      </table>
  ...
  return resend.emails.send({
    ...
    html: layout(`
      ...
      <div style="background:#18181C;border:1px solid #2A2A32;border-radius:10px;padding:14px 18px;margin:0 0 24px;font-family:monospace;font-size:12px;line-height:2;color:#B8B4AC;">
        <div>${AUTH_CENTER_ADDRESS.name}</div>
        <div>${AUTH_CENTER_ADDRESS.street1}</div>
        <div>${AUTH_CENTER_ADDRESS.city}, ${AUTH_CENTER_ADDRESS.state} ${AUTH_CENTER_ADDRESS.zip}</div>
      </div>
...
export async function emailStrikeAppealSubmitted({ strike, seller, reason }) {
  const ownerEmail = process.env.OWNER_EMAIL
  if (!ownerEmail) return
  return resend.emails.send({
    from: FROM, to: ownerEmail,
    subject: `Strike appeal submitted — ${seller?.username || seller?.full_name || 'Seller'} — Chase Hollow`,
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Strike <em style="color:#E8A838;">Appeal</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 24px;">A seller has submitted an appeal for Strike #${strike.strike_number}. Review and decide within 48 hours.</p>
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;margin-bottom:20px;" cellpadding="8">
        <tr><td style="color:#6C6A66;font-size:13px;">Seller</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${seller?.username || seller?.full_name || '—'}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Strike #</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${strike.strike_number}</td></tr>
        <tr><td style="color:#6C6A66;font-size:13px;">Reason on file</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${strike.reason || '—'}</td></tr>
      </table>
      <div style="background:#1A1A1F;border-left:3px solid #E8A838;padding:12px 16px;border-radius:0 8px 8px 0;margin-bottom:24px;">
        <div style="color:#6C6A66;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">Seller's appeal reason</div>
        <div style="color:#F0EDE6;font-size:13px;line-height:1.6;">${reason}</div>
      </div>
      ${btn('Review in Admin', `${BASE_URL}/admin`)}
    `)
  })
}

// ── Auth fail return emails ───────────────────────────────────────────────────

export async function emailBuyerAuthFailReturnLabel({ to, order }) {
  return resend.emails.send({
    from: FROM, to,
    subject: 'Authentication failed — prepaid return label inside — Chase Hollow',
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Authentication <em style="color:#C84B3C;">Failed</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 16px;">
        Our team was unable to verify the authenticity of the card in order <strong style="font-family:monospace;color:#F0EDE6;">#${order.id.slice(0,8).toUpperCase()}</strong>. The card has been delivered to you, but you are entitled to a <strong style="color:#F0EDE6;">full refund</strong>.
      </p>
      <div style="background:#1a0d0d;border:1px solid rgba(200,75,60,0.4);border-radius:10px;padding:16px 20px;margin:0 0 20px;">
        <p style="color:#C84B3C;font-size:14px;font-weight:700;margin:0 0 4px;">Action required — ship the card back within 5 days</p>
        <p style="color:#B8B4AC;font-size:13px;margin:0;">A prepaid return label has been added to your buyer dashboard. Print it, pack the card securely, and drop it with the carrier. Your refund will be processed automatically once delivery is confirmed.</p>
      </div>
      <p style="color:#B8B4AC;font-size:13px;margin:0 0 24px;">The seller has been penalized. If you have questions, contact Chase Hollow support.</p>
      ${btn('Get Return Label', `${BASE_URL}/buyer-dashboard`)}
    `)
  })
}

export async function emailBuyerAuthFailRefunded({ to, order }) {
  return resend.emails.send({
    from: FROM, to,
    subject: 'Refund confirmed — Chase Hollow',
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Refund <em style="color:#4CAF7C;">Confirmed</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 16px;">
        Your returned card has been received and your full refund for order <strong style="font-family:monospace;color:#F0EDE6;">#${order.id.slice(0,8).toUpperCase()}</strong> has been processed to your wallet.
      </p>
      <p style="color:#B8B4AC;font-size:13px;margin:0 0 24px;">USDC is on its way. On-chain transfers typically confirm within a few minutes. Thank you for your patience — we take card authenticity seriously.</p>
      ${btn('View Dashboard', `${BASE_URL}/buyer-dashboard`)}
    `)
  })
}

export async function emailStrikeAppealResolved({ to, fullName, decision, strikeNumber }) {
  const approved = decision === 'approved'
  const firstName = (fullName || '').split(' ')[0]
  return resend.emails.send({
    from: FROM, to,
    subject: `Strike appeal ${approved ? 'approved' : 'denied'} — Chase Hollow`,
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Appeal <em style="color:${approved ? '#4CAF7C' : '#C84B3C'};">${approved ? 'Approved' : 'Denied'}</em></h2>
      ${firstName ? `<p style="color:#F0EDE6;font-size:14px;margin:0 0 16px;">Hi ${firstName},</p>` : ''}
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 24px;">
        ${approved
          ? `Your appeal for Strike #${strikeNumber} has been approved. The strike has been removed from your account and any suspension has been lifted.`
          : `Your appeal for Strike #${strikeNumber} has been reviewed and denied. The strike remains on your account. Please review our seller policies to avoid future strikes.`}
      </p>
      ${btn('View Dashboard', `${BASE_URL}/seller-dashboard`)}
    `)
  })
...
// Day-7 gentle check-in for self-ship orders with no confirmed delivery
export async function emailBuyerSelfShipDay7({ to, order }) {
  const releaseDate = new Date(order.auto_release_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  const tracking = order.tracking_a
  const carrier  = order.self_ship_carrier
  return resend.emails.send({
    from: FROM, to,
    subject: 'Has your card arrived? — Chase Hollow',
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Has Your Card <em style="color:#C9A84C;">Arrived?</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 16px;">
        It's been a week since the seller shipped order <strong style="font-family:monospace;color:#F0EDE6;">#${order.id.slice(0,8).toUpperCase()}</strong>. We just want to make sure everything is on track.
      </p>
      ${tracking ? `
      <table width="100%" style="border:1px solid #2A2A32;border-radius:10px;padding:16px;margin:0 0 20px;" cellpadding="8">
        ${carrier ? `<tr><td style="color:#6C6A66;font-size:13px;">Carrier</td><td style="color:#F0EDE6;font-size:13px;text-align:right;">${carrier}</td></tr>` : ''}
        <tr><td style="color:#6C6A66;font-size:13px;">Tracking</td><td style="color:#F0EDE6;font-size:13px;text-align:right;font-family:monospace;">${tracking}</td></tr>
      </table>` : ''}
      <div style="background:rgba(201,168,76,0.06);border:1.5px solid rgba(201,168,76,0.25);border-radius:10px;padding:16px 20px;margin:0 0 20px;">
        <p style="color:#C9A84C;font-size:13px;font-weight:600;margin:0 0 4px;">Card received? — great, nothing to do.</p>
        <p style="color:#B8B4AC;font-size:13px;margin:0;">You can confirm receipt early in your dashboard to start your 72-hour inspection window. Funds auto-release on <strong style="color:#F0EDE6;">${releaseDate}</strong> if no action is taken.</p>
      </div>
      <div style="background:rgba(200,75,60,0.06);border:1.5px solid rgba(200,75,60,0.25);border-radius:10px;padding:16px 20px;margin:0 0 24px;">
        <p style="color:#C84B3C;font-size:13px;font-weight:600;margin:0 0 4px;">Card not received?</p>
        <p style="color:#B8B4AC;font-size:13px;margin:0;">If delivery is delayed or the tracking isn't updating, open a dispute now from your dashboard. Acting early gives us the best window to investigate.</p>
      </div>
      ${btn('View Order', `${BASE_URL}/buyer-dashboard`)}
    `)
  })
...
// Day-12 urgent warning — auto-release is 2 days away
export async function emailBuyerSelfShipDay12({ to, order }) {
  const releaseDate = new Date(order.auto_release_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  return resend.emails.send({
    from: FROM, to,
    subject: 'Action needed — funds release soon — Chase Hollow',
    html: layout(`
      <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-size:26px;font-weight:400;color:#F0EDE6;">Funds Release <em style="color:#C84B3C;">Soon</em></h2>
      <p style="color:#B8B4AC;font-size:14px;line-height:1.6;margin:0 0 16px;">
        Order <strong style="font-family:monospace;color:#F0EDE6;">#${order.id.slice(0,8).toUpperCase()}</strong> is set to auto-release to the seller on <strong style="color:#C84B3C;">${releaseDate}</strong>. We still have no confirmed delivery on file.
      </p>
      <div style="background:rgba(200,75,60,0.08);border:2px solid rgba(200,75,60,0.4);border-radius:10px;padding:20px;margin:0 0 24px;">
        <p style="color:#C84B3C;font-size:14px;font-weight:700;margin:0 0 8px;">If you have not received your card — open a dispute immediately.</p>
        <p style="color:#B8B4AC;font-size:13px;margin:0;">Once funds are released the transaction is final. Your dispute window closes on ${releaseDate}. Do not wait.</p>
      </div>
      <p style="color:#B8B4AC;font-size:13px;margin:0 0 24px;">If your card has arrived and everything looks good, you can confirm receipt and release funds early from your dashboard.</p>
      ${btn('Open Dispute / Confirm Receipt', `${BASE_URL}/buyer-dashboard`)}
    `)
  })
}
